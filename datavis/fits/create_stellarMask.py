...
"""create_stellarMask.py -- Create a fits file that can act as a stellar mask for input fits image. Mask will have zeros and ones, ones are the locations where there are stars. The mask is created by growing the source extractor segmentation map. 

Required input 
    fitsimage - image for which a stellar mask is to be created.

Usage:
    create_stellarMask.py [-h] [-v] [--debug] [-o LOC] [-s LOC] <fitsimage>
...
Options:
    ...
    -o LOC, --out LOC               Output mask fits file here [default: ./stellar_mask.fits]
    -s LOC, --sextractor LOC        SExtractor location [default: /opt/local/bin/source-extractor]   

Example:
    python create_stellarMask.py -v fitsimage.fits
"""
...
a little larger and used as the aggressive stellar mask.
'''
...
def create_stellarMask(fitsimage,output=False,sextractorloc='/opt/local/bin/source-extractor',verbose=False,debugmode=False):
    '''Create a fits file that can act as a stellar mask for input fits image. 
    ...
    if output:
        if os.path.exists(output):
            printme = f'ERROR  : cannot overwrite existing file: {output}. Delete/rename/specify a different output name before trying agian.'
            sys.exit(printme)
...
if __name__=='__main__':
    ...
    _ = create_stellarMask(  fitsimage, output = output, sextractorloc = sextractorloc, verbose = verbose, debugmode = debugmode)
