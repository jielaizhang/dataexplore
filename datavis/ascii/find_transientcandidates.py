...
def select_rightRADEC(df_sci,df_pos,df_neg,RAmin,RAmax,DECmin,DECmax,verbose=False):
    ...
    return df_sci_RADECselect,df_pos_RADECselect,df_neg_RADECselect
...
def find_transientCandidates_(sub_cat, sci_cat, neg_cat, 
    ...
    df_sci_RADECselect,df_pos_RADECselect,df_neg_RADECselect = select_rightRADEC(df_sci,df_pos,df_neg,RAmin,RAmax,DECmin,DECmax,verbose=verbose)
    ...
    # Round 2: for each science detection there is a positive detections associated
    ...
    # If sub catalog successfully read in, and this function wasn't terminated, there will be sources to write into these region files.

    # Sub Detections Selected (Green squares)
    Rs = np.array(df_pos_select['X_WORLD'])
    Ds = np.array(df_pos_select['Y_WORLD'])
    Widths = ["0.09'"]*len(Rs)
    Heights = ["0.00'"]*len(Rs)
    Angles = ["0.0"]*len(Rs)
    Shapes = ['box']*len(Rs)
    ...
    Rs = np.array(df_neg_select['X_WORLD'])
    Ds = np.array(df_neg_select['Y_WORLD'])
    Widths = ["0.11'"]*len(Rs)
    Heights = ["0.11'"]*len(Rs)
    Angles = ["0.0"]*len(Rs)
    Shapes = ['box']*len(Rs)
    ...
    Rs = np.array(df_sci_select['X_WORLD'])
    Ds = np.array(df_sci_select['Y_WORLD'])
    Widths = ["0.11'"]*len(Rs)
    Heights = ["0.11'"]*len(Rs)
    Angles = ["30.0"]*len(Rs)
    Shapes = ['box']*len(Rs)
    ...
    elif no_sciYes_posYes_candidates == True:
    ...
    # Add ds9 command to ds9 commands file: all detections
    ...
    # Add ds9 command to ds9 commands file: sciYes subYes
    if no_sciYes_posYes_candidates == False:
    ...
    # Add ds9 command to ds9 commands file: sciYes subYes negNo
    if no_sciYes_posYes_negNo_candidates == False:
    ...
    # Add ds9 command to ds9 commands file: sciYes subYes negNo, but sub sci only
    if no_sciYes_posYes_negNo_candidates == False:
    ...
    # ..............................................................
    # Create sciYes subYes noNeg candidates list
    ...
    elif no_sciYes_posYes_negNo_candidates == True:
        # Write empty candidate list file
        fp = open(candidate_list_file, "w")
        fp.write('# No candidates detected.\n')
        fp.write(h)
